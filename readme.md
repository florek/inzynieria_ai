...
Aktualny zakres notatek obejmuje m.in. **wstęp, rozdział 1 w całości oraz rozszerzony fragment rozdziału 2** opracowania w materiale źródłowym (paginacja jak w podglądzie **Strona N** w pliku HTML wygenerowanym z PDF), zsynchronizowany z eksporterem PDF do **linii 1442** w tym pliku HTML, tj. m.in. dane treningowe, wielojęzyczność, architekturę transformera i alternatywy, rozmiar modelu wraz z szacunkiem pamięci, rzadkością wag, architekturą MoE oraz rozróżnieniem tokenów w zbiorze danych i tokenów treningowych.
